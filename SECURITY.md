...
where untrusted data enters (or workspace content leaves), and what it deliberately
does not defend against — is documented in
[`docs/THREAT-MODEL.md`](docs/THREAT-MODEL.md). This file is the operator-facing
policy: how to report an issue, how to verify a release, and how to handle your
key and cache safely.
...
Every release artifact — the archives, the `.deb` and `.rpm` packages, and
`checksums.txt` — carries [SLSA build provenance](https://slsa.dev) attested by
GitHub (keyless Sigstore). To verify a downloaded artifact really was built by
...
# e.g.
gh attestation verify linearfs_<version>_linux_amd64.tar.gz -R jra3/linear-fuse
gh attestation verify linearfs_<version>_linux_amd64.deb -R jra3/linear-fuse
```

Verify the package *before* handing it to `apt`/`dnf` — installing it runs its
scripts as root.

`checksums.txt` guards against download corruption, but it is generated by the same
job that builds the binaries — provenance verification is what detects a swapped
artifact.
