...
// (body). It owns the change decision (what counts as "changed") and the
// read-your-writes divergence classification, so the two handlers no longer
// each hand-roll a `fieldChanged` flag and a byte-identical commitWriteBack
// compare closure. See CONTEXT.md "Scalar edit (scalarEdit)".
//
...
// name/description (marshal.MarkdownToProjectEdit/MarkdownToInitiativeEdit own
// the extraction and the name coercion). The body maps to the description;
// both sides are trimmed for the change test so a render/parse
// trailing-newline delta doesn't read as an edit, and the trimmed body is what
// we send. An empty or unchanged name is left alone.
