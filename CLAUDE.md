...
- **internal/fs**: FUSE filesystem implementation using go-fuse/v2. Each node type implements `fs.Node*` interfaces:
  - `RootNode` → `/` with `teams/` and `my/` directories
  - `TeamsNode`/`TeamNode` → `/teams/<KEY>/`
  - `MyNode`/`MyAssignedNode` → `/my/assigned/`
  - `IssueNode` → Individual issue files (read/write)
  - `NewIssueNode` → Handles file creation for new issues
...
1. `IssueNode.Write()` buffers changes
2. `IssueNode.Flush()` (on save) parses markdown via `marshal.MarkdownToIssueUpdate`
