...
| `linearfs.sync.probe_outcomes` | counter | `kind` = `team_projects` \| `initiatives`, `outcome` = `unchanged` \| `changed` \| `error` | one record per change-detection probe run (`probeTeamProjects`, #243). `unchanged` = the newest-first page carried nothing past the persisted watermark (the ~1K steady-state check); `changed` = the resume walk upserted ≥1 project and advanced the watermark; `error` = fetch/upsert failure or cancellation, watermark untouched. Probes ride lean cycles, **plus** a full cycle's per-drain fallback when that drain was deferred (`cycle_duration{mode=full,outcome=deferred}`) — the probes bypass the `LowBudget` preflight, so they still admit at ~1K where the drain was refused. So a probe that never fires shows up as this series going flat while `cycle_duration` keeps sampling *any* mode. The initiatives probe (`probeInitiatives`, #244) records the same outcomes; its `changed` escalates the full workspace sync |
