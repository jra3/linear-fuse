...
| Instrument | Kind | Attributes | Recorded |
|---|---|---|---|
| `linearfs.sync.cycle_duration` | histogram (s) | `mode` = `lean` \| `full`, `outcome` (full only) = `complete` \| `deferred` \| `failed` | defer-recorded inside `syncCycle`, one sample per cycle for all three invokers (initial run, ticker, `SyncNow`). `mode` is the cycle's speed (#242): `full` = workspace + team-metadata drains + issues (cold start, `SyncNow`, and every `FullSyncInterval`, default 10m, off the persisted `sync_schedule` timestamp); `lean` = incremental issues + the change-detection probes (per-team projects #243, initiatives #244), skipping the `GetWorkspace`/`GetTeamMetadata` fetches. The per-mode sample counts double as the cycle-mode counter — no separate instrument. Full cycles also carry `outcome` = `complete` \| `deferred` \| `failed`: what their skeleton-tier drains did, and therefore whether the cycle stamped `sync_schedule`. `deferred` (the admission ladder or the `LowBudget` preflight refused a drain) **withholds** the stamp, so the full sync stays due; `failed` (network, GraphQL, permissions) stamps anyway, because retrying it is expensive where retrying a deferral is nearly free. Read the per-series **count** — the default bucket bounds start at 5 against values in SECONDS, so a ~0s deferred cycle and a healthy 3s one share a bucket. Lean cycles carry no `outcome` (no drain, no stamp) |
...
| `linearfs.sync.probe_outcomes` | counter | `kind` = `team_projects` \| `initiatives`, `outcome` = `unchanged` \| `changed` \| `error` | one record per change-detection probe run (`probeTeamProjects`, #243). `unchanged` = the newest-first page carried nothing past the persisted watermark (the ~1K steady-state check); `changed` = the resume walk upserted ≥1 project and advanced the watermark; `error` = fetch/upsert failure or cancellation, watermark untouched. The projects probe rides lean cycles **plus** a full cycle's per-team fallback when that team's metadata drain was deferred (`cycle_duration{mode=full,outcome=deferred}`) — it bypasses the `LowBudget` preflight, so it still admits at ~1K where the drain was refused. So a `team_projects` probe that never fires shows up as that series going flat while `cycle_duration` keeps sampling *any* mode. The initiatives probe (`probeInitiatives`, #244) records the same outcomes and its `changed` escalates the full workspace sync — but it is lean-cycles-only, with no deferred-drain fallback (the escalation would hit the very drain that was refused), so its series is expected to go flat for a starved window |
...
{"ts":"2026-07-09T02:00:07.900000000Z","op":"IssueCreate",
 ...
 "error":{"message":"Argument Validation Error","code":"","type":"","user_error":false,
          "user_presentable_message":"name must be at most 80 characters","errors":1}}
```
...
- `error` — **present only on a failed request**; the rejection, decoded (#448).
  ...
  which is the question an after-the-fact investigation actually asks. Its six
  ...
  - `errors` — how many entries the response's `errors` array carried. Only the
    **first** is decoded above (it is also the one that becomes the returned Go
...
An untagged rejection in that output with `errors` above 1 is the one case the
census cannot take at face value: the tagged entry Linear also sent was dropped
...
of the same API key). `BudgetSnapshot` is now the client's own log line only —
the sync worker's `BudgetReporter` seam and its two percentage thresholds were
deleted, since a requests-axis percentage cannot govern a complexity-bound
workload and admission belongs to `ratebudget.admit` alone. `config.Log.APIStats` was removed; old config files
