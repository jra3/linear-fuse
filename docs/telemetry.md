...
| Instrument | Kind | Attributes | Recorded |
|---|---|---|---|
| `linearfs.sync.cycle_duration` | histogram (s) | `mode` = `lean` \| `full` | defer-recorded inside `syncCycle`, one sample per cycle for all three invokers (initial run, ticker, `SyncNow`). `mode` is the cycle's speed (#242): `full` = workspace + team-metadata drains + issues (cold start, `SyncNow`, and every `FullSyncInterval`, default 10m, off the persisted `sync_schedule` timestamp); `lean` = incremental issues plus the cheap change-detection probes (#244), skipping the `GetWorkspace`/`GetTeamMetadata` fetches unless a probe detects change. The per-mode sample counts double as the cycle-mode counter — no separate instrument. Budget-skipped cycles record ~0s (attributed with the mode they would have run) — that near-zero spike IS the skip signature |
...
| `linearfs.sync.probe_outcomes` | counter | `kind` = `initiatives`, `outcome` = `unchanged` \| `changed` \| `error` | one per lean-cycle change-detection probe run (#244, `probeInitiatives`): `unchanged` = newest `updatedAt` ≤ persisted watermark, the expensive fetch was skipped; `changed` = the on-change full sync was triggered (whether or not it then succeeded — its failures log separately); `error` = the probe query itself failed. Full cycles never probe, so a healthy steady state shows ~4 probe runs per full cycle. A probe that never fires shows up as a missing `kind` |
